...
namespace Admin.Services.Purchase.Apply.BusinessLayer.Services
{
    public class PvaYearOfRegService : IPvaYearOfRegService
    {
        ...
        public PvaYearOfRegService(DealerApifinalContext db, IMapper mapper)
        {
            _db = db;
            _mapper = mapper;
        ...
        public async Task<PvaYearOfRegDto> AddYearAsync(PvaYearOfRegDto newStateDto)
        {
            try
            {
                ...
                var newState = _mapper.Map<PvaYearOfRegtbl>(newStateDto);

                // Add the new state to the database
                _db.PvaYearOfRegtbls.Add(newState);
                await _db.SaveChangesAsync();

                // Map the added entity back to DTO
                var addedStateDto = _mapper.Map<PvaYearOfRegDto>(newState);
        ...
        public async Task<IActionResult> DeleteYearAsync(int id)
        {
            try
            {
                // Find the state in the database by id
                var stateToDelete = await _db.PvaYearOfRegtbls.FindAsync(id);
                ...
                _db.PvaYearOfRegtbls.Remove(stateToDelete);
                await _db.SaveChangesAsync();
        ...
        public async Task<PvaYearOfRegDto> GetYearByIdAsync(int id)
        {
            try
            {
                var getState = await _db.PvaYearOfRegtbls.FindAsync(id);
                ...
                var showState = _mapper.Map<PvaYearOfRegDto>(getState);
        ...
        public async Task<IEnumerable<PvaYearOfRegDto>> GetYearDetailsAsync()
        {
            try
            {
                var statedetails = await _db.PvaYearOfRegtbls.ToListAsync();

                if (statedetails == null || statedetails.Count == 0)
                {
                    // Return an empty collection if no data is found
                    return new List<PvaYearOfRegDto>();
                }

                // Use AutoMapper to map the entities to DTO
                var statesDto = _mapper.Map<IEnumerable<PvaYearOfRegDto>>(statedetails);
        ...
        public async Task<PvaYearOfRegDto> UpdateYearAsync(int id, PvaYearOfRegDto updatedStateDto)
        {
            try
            {
                ...
                // Find the state in the database by id
                var existingState = await _db.PvaYearOfRegtbls.FindAsync(id);
                ...
                _db.PvaYearOfRegtbls.Update(existingState);
                await _db.SaveChangesAsync();

                // Map the updated entity back to DTO
                var updatedStateDtoResult = _mapper.Map<PvaYearOfRegDto>(existingState);
